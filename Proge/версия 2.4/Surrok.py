...
def lokatsee_nora():
    ...
    canvas.create_image(140, 60, image = photo_list_fon[16], tags = 'Nadpes_knopke')
    x, y = 540, 320
    ...
    if boy_active:
        if event.keysym == 'Up':
            ...
            for ataka in canvas.find_withtag('ataka'): # ищит среди всех тегов только теги ataka
                ...
                if (position_ataka[0] - 35 < position_ser[0] < position_ataka[0] + 35 and
                    position_ataka[1] - 35 < position_ser[1] < position_ataka[1] + 35):
                    canvas.delete(ataka)
                    break
...
def boy_komare():
    global x1, y1, b_komare, ochke, id_ochke_count_label, id_etap_count_label
    start_boy()
    b_komare = 1
    canvas.delete("all")
...
def boy_korsak():
    global x1, y1, b_korsak, ochke, id_ochke_count_label, id_etap_count_label
    start_boy()
    b_korsak = 1
    canvas.delete("all")
...
def boy_zmeia():
    global x1, y1, b_zmeia, ochke, id_ochke_count_label, id_etap_count_label
    start_boy()
    b_zmeia = 1
    canvas.delete("all")
...
def boy_orel():
    global x1, y1, b_orel, ochke, id_ochke_count_label, id_etap_count_label
    start_boy()
    b_orel = 1
    canvas.delete("all")
...
def ataka_1(): # функция (этап 1)
    global boy_active, id_ochke_count_label, ochke, id_etap_count_label, b
    if boy_active:
       k = 0
       while b != 0:
           ochke = 5
...
def ochke_zezne(): # функция (подсчета очков)
    global ochke, id_ochke_count_label
    if ochke == 4:
       id_ochke_count_label.destroy()
...
canvas.bind_all("<Enter>",start_end)
