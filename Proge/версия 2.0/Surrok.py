...
photo_list = [
    ...
    tk.PhotoImage(file="Photo/Okno_boia.png"),# 16                ДЛЯ БОЯ
    tk.PhotoImage(file="Photo/Serdechko.png"),# 17
    # tk.PhotoImage(file="Photo/Ochke_zezne.png"),# 18
    # tk.PhotoImage(file="Photo/Etap_boia.png"),# 19
    # tk.PhotoImage(file="Photo/Ataka.png")# 20
]
x, y = 500, 350
x1, y1 = 480, 450
k1 = 5
...
def boy_komare():
    ...
    id_okno_boia = canvas.create_image(480, 450, image=photo_list[16],
                                       tags='okno_boia')
    # for i in range(0, len(id_predmets_4), 2):  # от 0 до 10 шаг 2 ([x,y], [x,y], [x,y], [x,y], [x,y] - лист предметов)
    #     predmets_4.append(canvas.create_image(id_predmets_4[i], id_predmets_4[i + 1], image=photo_list[4],
    #                                           tags='predmets_4'))
    # x, y = 480, 470
    # id_surok_128 = canvas.create_image(x, y, image=photo_list[0], tags='surok_128')
    id_serdechko = canvas.create_image(x1, y1, image=photo_list[17], tags='serdechko')  # вывод сурка
...
def move_serdechko(event): # функция event-события (движение сердечка)
    ...
    if event.keysym == 'Up':
        ...
        id_serdechko = canvas.create_image(x1,y1, image = photo_list[17], tags = 'serdechko')
...
def check_predmet():  # функция (сбор предмета сурком)
    global predmets_1,predmets_2,predmets_3,predmets_4,id_predmets_1, \
        id_predmets_2, id_predmets_3, id_predmets_4, k1, k2, k3, k4
    position_sur = canvas.coords('surok_128')
...
# def check_gde_boy(): # функция (проверка какой бой запустить)
#     global b
#     position_sur = canvas.coords('surok_128')
#     if position_sur[0] <= 480:
#      boy_komare()
    # while True:
    #     position_sur = canvas.coords('surok_128')
    #     if position_sur[0] <= 480:
...
def check_gran_2(): # функция (не заходить за границы)
    ...
    if position_ser[0]+5 > 550: # правая граница
       canvas.coords('serdechko', 550, position_ser[1]) # 900 - на сколько откатить
       # ([0] - позиция по x) + (60 - ширина [подобрана] сурка по x) > (555 - размер экрана)
       x1 = 550
    if position_ser[0]-5 < 410: # левая граница
       canvas.coords('serdechko', 410, position_ser[1])
       x1 = 410
    if position_ser[1]+5 > 520: # нижняя граница
       canvas.coords('serdechko', position_ser[0], 520)
       y2 = 520
    if position_ser[1]-5 < 375: # верхняя граница
       canvas.coords('serdechko', position_ser[0], 375)
       y2 = 375
# КАК ИСПРАВИТЬ ЧТОБЫ ВУГЛАХ НОРМАЛЬНО ПЕРЕДВИГАЛСЯ, СУРРОК ПОЯВЛЯЕТСЯ В БОЮ -------------------------------------
...
# Проблема: ВТОРАЯ КОРЗИНА НАКЛАДВАЕТСЯ НА СУРКА
